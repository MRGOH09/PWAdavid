...
class PWAClient {
  ...
  async call(endpoint, action, params = {}, options = {}) {
    ...
    try {
      ...
      const url = `${this.getBaseURL()}/api/pwa/${endpoint}`
      ...
      if (!response.ok) {
        ...
        // 405 兼容处理：自动退化为 GET 带 query 参数重试（适配旧缓存/旧API）
        if (response.status === 405) {
          try {
            const query = new URLSearchParams({ action, ...Object.fromEntries(Object.entries(params).map(([k,v]) => [k, String(v)])) }).toString()
            const retry = await fetch(`${url}?${query}`, {
              method: 'GET',
              headers,
              credentials: 'include'
            })
            if (retry.ok) {
              ...
              if (useCache && data2) smartCache.set(endpoint, action, params, data2)
              return data2
            }
          } catch (e) {
